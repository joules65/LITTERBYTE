...
import { Calendar1Icon, WeightIcon, Search, Clock, Trash, CheckCircle2, Loader, Upload, Weight, Calendar, MapPin } from "lucide-react"
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import toast from 'react-hot-toast'
import { GoogleGenerativeAI } from '@google/generative-ai'
import { getUserByEmail, getWasteCollectionTask, saveCollectedWaste, saveReward, updateTaskStatus } from '@/utils/database/actions'
...
type CollectTask = {
    ...
    wasteType:string;
    amount: string;
    status:"pending" | "completed"|"in_progress"|"verified";
    date: string;
...
export default function CollectPage() {
    ...
    const handleStatusChange = async (taskId: number, newStatus: CollectTask["status"]) => {
        if(!user) {
    ...
    const handleVerification = async () => {
        if(!selectedTask || !verificationImage || !user) {
            toast.error("Please select a task, upload a verification image, and log in.");
            return;
        }
      ...
      if (!base64Data || !base64Data.startsWith('/9j/')) { // Example check for JPEG
        console.error('Invalid base64 data for the image');
      ...
      const prompt = `You are an expert in waste management and recycling. Analyze this image and provide:
        ...
        2. Estimate if the amount of waste shown is comparable to the required amount: ${selectedTask.amount} Describe if it looks greater, lesser, or approximately equal.
    ...
    const filteredTasks = tasks.filter(task =>
      ...
      return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
          <h1 className="text-3xl font-semibold mb-6 text-lime-900 text-center">Waste Collection Tasks</h1>
                  ...
                  <div key={task.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                    <div className="flex justify-between items-center mb-2">
                      <h2 className="text-lg font-medium text-gray-800 flex items-center">
                        <MapPin className="w-5 h-5 mr-2 text-green-950" />
                        {task.location}
                      </h2>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-sm text-gray-600 mb-3">
                      <div className="flex items-center relative">
                        <Trash className="w-4 h-4 mr-2 text-green-500" />
                        <span 
                        ...
                        {hoveredWasteType === task.wasteType && (
                          <div className="absolute left-0 top-full mt-1 p-2 bg-black text-white text-xs rounded shadow-lg z-10">
                            {task.wasteType}
                          </div>
                        )}
                      </div>
          ...
          {selectedTask && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-4">
                  <label htmlFor="verification-image" className="block text-sm font-medium text-green-700 mb-2">
                    Upload Image
                  </label>
    ...
    function StatusBadge({ status }: { status: CollectTask['status'] }) {
      const statusConfig = {
        ...
        verified: { color: 'bg-purple-100 text-purple-800', icon: CheckCircle2 },
      }
